...
public class Address
{
    ...
    private static final int CITY_NAME_LENGTH_THRESHOLD = 1;
     ...
     */
    public Address(final String unitNumber, final int streetNumber, final String streetName,
                   final String postalCode, final String cityName)
    {
        if(unitNumber != null)
        {
            if(unitNumber.length() < VALID_MIN_UNIT_NUMBER_LENGTH ||
                    unitNumber.length() > VALID_MAX_UNIT_NUMBER_LENGTH)
            {
        ...
        if(streetName != null)
        {
            if (streetName.length() < VALID_MIN_STREET_NAME_LENGTH ||
                    streetName.length() > VALID_MAX_STREET_NAME_LENGTH)
            {
        ...
        if(postalCode != null)
        {
            if(postalCode.length() < VALID_MIN_POSTAL_CODE_LENGTH ||
                    postalCode.length() > VALID_MAX_POSTAL_CODE_LENGTH)
            {
        ...
        if(cityName != null)
        {
            if (cityName.length() < VALID_MIN_CITY_NAME_LENGTH ||
                    cityName.length() > VALID_MAX_CITY_NAME_LENGTH)
            {
    ...
    public String getFormattedCityName()
    {
        ...
        if(cityNameData.length > CITY_NAME_LENGTH_THRESHOLD)
        {
        ...
        else if(cityNameData.length == CITY_NAME_LENGTH_THRESHOLD)
        {
            formattedCityName = cityNameData[0].substring(0, 1).toUpperCase() +
                    cityNameData[0].substring(1).toLowerCase();
        }
    ...
    /**
     * @return The Address class is represented by a String, displaying all attributes in Address class.
     */
    @Override
